...
class DeleteCashier : AppCompatActivity() {
            ...
            if (usernameToDelete.isNotEmpty()) {
                ...
                val deleteDialog = DeleteUserConfirmDialog(
                    ...
                    onDeleteConfirmed = {
                        ...
                        val deleted = dbHandler.deleteUser(DBHandler.TABLE_CASHIER, usernameToDelete) // Deleting from Cashier table
                        if (deleted) {
                            Toast.makeText(this, "Cashier deleted successfully", Toast.LENGTH_SHORT).show()
                        } else {
